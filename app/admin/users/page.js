...
export default function UsersPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setShowModal(false);
        setEditingUser(null);
        setFormData({ username: '', password: '', nama: '', role: 'peminjam' });
        fetchUsers();
      } else {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Yakin ingin menghapus ${selectedItems.length} data user yang dipilih?`)) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="ml-64 flex-1 flex flex-col min-h-screen">
        <Header user={user} title="CRUD User" />
        <main className="flex-1 p-8 overflow-auto">
          <div className="max-w-7xl mx-auto">
            {/* Header Section */}
            <div className="mb-8 flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">CRUD User</h1>
            ...
            <div className="mb-8">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Cari berdasarkan ID, username, nama, atau role..."
                  value={searchQuery}
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-12">
                        <input
                          type="checkbox"
                          checked={paginatedUsers.length > 0 && selectedItems.length === paginatedUsers.length}
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {paginatedUsers.length === 0 ? (
                        ...
                        <td colSpan="6" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-sm text-gray-500">
                              {searchQuery ? 'Tidak ada data yang sesuai dengan pencarian' : 'Tidak ada data user'}
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      paginatedUsers.map((u) => (
                        <tr key={u.id} className={`hover:bg-blue-50/50 transition-colors ${selectedItems.includes(u.id) ? 'bg-blue-50' : ''}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{u.nama}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1.5 rounded-full text-xs font-semibold ${
                              u.role === 'admin' ? 'bg-purple-100 text-purple-800' :
                              u.role === 'petugas' ? 'bg-blue-100 text-blue-800' :
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      Password {editingUser && <span className="text-xs text-gray-500 font-normal">(kosongkan jika tidak diubah)</span>}
                    </label>
                    <input
                      type="password"
