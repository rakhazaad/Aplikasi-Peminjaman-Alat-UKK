...
export default function KategoriPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingKategori
        ? `/api/admin/kategori/${editingKategori.id}`
        : '/api/admin/kategori';
  ...
  const handleEdit = (kat) => {
    ...
    setFormData({ nama: kat.nama, deskripsi: kat.deskripsi || '' });
    setShowModal(true);
  };
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(paginatedKategori.map(kat => kat.id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Yakin ingin menghapus ${selectedItems.length} data kategori yang dipilih?`)) return;

    try {
      const deletePromises = selectedItems.map(id => 
        fetch(`/api/admin/kategori/${id}`, { method: 'DELETE' })
      );
  ...
  const filteredKategori = useMemo(() => {
    if (!searchQuery.trim()) return kategori;
    const query = searchQuery.toLowerCase();
    return kategori.filter(kat => 
      kat.nama?.toLowerCase().includes(query) ||
      kat.deskripsi?.toLowerCase().includes(query) ||
      kat.id.toString().includes(query)
    );
  ...
  const paginatedKategori = filteredKategori.slice(startIndex, endIndex);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="ml-64 flex-1 flex flex-col min-h-screen">
        <Header user={user} title="CRUD Kategori" />
        <main className="flex-1 p-8 overflow-auto">
          <div className="max-w-7xl mx-auto">
            {/* Header Section */}
            <div className="mb-8 flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">CRUD Kategori</h1>
            ...
            <div className="mb-8">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Cari berdasarkan ID, nama, atau deskripsi..."
                  value={searchQuery}
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-12">
                        <input
                          type="checkbox"
                          checked={paginatedKategori.length > 0 && selectedItems.length === paginatedKategori.length}
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {paginatedKategori.length === 0 ? (
                        ...
                        <td colSpan="5" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-sm text-gray-500">
                              {searchQuery ? 'Tidak ada data yang sesuai dengan pencarian' : 'Tidak ada data kategori'}
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      paginatedKategori.map((kat) => (
                        <tr key={kat.id} className={`hover:bg-blue-50/50 transition-colors ${selectedItems.includes(kat.id) ? 'bg-blue-50' : ''}`}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <input
                              type="checkbox"
                              checked={selectedItems.includes(kat.id)}
                              onChange={() => handleSelectItem(kat.id)}
                              className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{kat.id}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{kat.nama}</td>
                          <td className="px-6 py-4 text-sm text-gray-600">{kat.deskripsi || '-'}</td>
            ...
            {filteredKategori.length > 0 && (
              <div className="mt-6 px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-b-2xl">
                <div className="text-sm text-gray-600 font-medium">
                  Menampilkan {startIndex + 1} - {Math.min(endIndex, filteredKategori.length)} dari {filteredKategori.length} data
                </div>
